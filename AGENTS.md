...
- 当前里程碑是 **UI Prototype 0.1**：React + TypeScript + Vite 的浏览器可运行交互原型。
- 正式产品只面向 Windows，目标链路为 `React Renderer -> Electron Main (Node.js) -> C# Word Worker -> Word COM`。
- 当前阶段不得把模拟转换、模拟环境检测或浏览器文件选择描述成已经接通 Electron/C# 的正式能力。
...
- `package.json` 是应用版本的单一真源；当前原型版本为 `0.1.0`。版本变化时同步 README 和里程碑说明。
