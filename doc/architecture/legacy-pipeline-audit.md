...
面向模板作者的步骤见 [模板制作指南](../../resources/docs/MD2Word-模板制作指南.md)。正文边界建议使用插入点书签；现有 Worker 替换范围是开始书签的 `Range.End` 到结束书签的 `Range.Start`。封面标题/副标题书签则应包住同一段落中的文字，不跨段落或包含图片；这是对既有实现的说明，不改变兼容契约。
